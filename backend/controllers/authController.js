...
export const register = async (req, res) => {
  ...
  try {
    ...
    res.json({
      token,
      user: { id: user._id, name, phone, phoneVerified, isAdmin: user.isAdmin, role: user.role },
    });
  } catch (err) {
...
export const login = async (req, res) => {
  ...
  try {
    ...
    if (!username && !user.phoneVerified) return res.status(400).json({ msg: 'Phone number not verified' });
    ...
    res.json({
      token,
      user: { id: user._id, name: user.name, phone: user.phone, username: user.username, phoneVerified: user.phoneVerified, isAdmin: user.isAdmin, role: user.role },
    });
  } catch (err) {
...
export const getAllUsers = async (req, res) => {
  try {
    // Fetch regular users (not admins/pos if preferred, or all)
    // For POS customers list, we typically want 'user' role.
